...
const books = [
  ...
  function getMostCommonGenres(books) {  
    ...
    ranks = books.reduce((acc, book) =>{
      if(acc.genre === book.genre) {
        acc.genre.count++;
      } else {
        acc.genre = book.genre;
        acc.genre.count = 1;
      }
      return acc;    
    },{}); 
  ...
  for(book of books) {        
    for(borrow of book.borrows){             
      if(borrow.id === selectID && borrow.returned === false){
        ...
        tempName = `${author.name.first} ${author.name.last}`        
...
console.log(getBooksPossessedByAccount(accounts.find(account=> account.id === "5f446f2e2cfa3e1d234679b9"), books, authors));
*/
...
function getBorrowersForBook(book, accounts) {  
  ...
  for(borrow of book.borrows){    
    ...
    result.push({name:account.name, email:account.email, returned:borrow.returned});
...
console.log(getBorrowersForBook(books.find(book => book.id ===  "5f447132d487bd81da01e25e"), accounts));